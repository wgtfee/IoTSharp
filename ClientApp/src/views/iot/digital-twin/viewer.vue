	...
	<div class="twin-viewer" v-loading="loading">
		<header>
			...
			<div class="viewer-actions">
				<el-segmented v-if="publishedView" v-model="runtimeMode" :options="runtimeModeOptions" @change="changeRuntimeMode" />
				<el-tag v-else type="warning" effect="dark">草稿模拟</el-tag>
				<el-tag v-if="runtimeMode === 'live'" type="success" effect="dark">实时接收中</el-tag>
				<el-button @click="router.push('/iot/digital-twin/scenes')">场景中心</el-button>
				<el-button v-if="sceneId" @click="router.push({ path:'/iot/digital-twin/workbench', query:{ sceneId } })">编辑草稿</el-button>
				<el-button v-if="runtimeMode === 'simulation'" @click="focusSimulationPallets">聚焦小托盘</el-button>
				<el-button v-if="runtimeMode === 'simulation'" type="primary" @click="toggleRunning">{{ running ? '暂停模拟' : '继续模拟' }}</el-button>
			</div>
		</header>
		<div ref="container" class="canvas"></div>
		<div class="metrics"><span>{{ runtimeMode === 'live' ? 'LIVE' : running ? 'SIMULATION RUNNING' : 'SIMULATION PAUSED' }}</span><span>{{ metrics.fps }} FPS</span><span>{{ metrics.triangles.toLocaleString('zh-CN') }} triangles</span><span v-if="runtimeMode === 'simulation' && metrics.routeSlots">小托盘 {{ metrics.routeSlots.plasticPallets }} · 可见 {{ metrics.routeSlots.visiblePlasticPallets }} · 独立位置 {{ metrics.routeSlots.uniquePlasticPositions }}</span><span v-if="runtimeMode === 'live'">遥测 {{ Object.keys(latestUpdates).length }} 条</span><span v-if="metrics.silkLine">托盘 {{ metrics.silkLine.onlinePallets }} · 等待 {{ metrics.silkLine.waitingPallets }}</span></div>
...
import { cloneTwinManifest, type TwinSceneManifest } from '/@/digital-twin/contracts';
...
type RuntimeDataMode = 'simulation' | 'live';
const runtimeMode = ref<RuntimeDataMode>('simulation');
const runtimeModeOptions = [{ label: '模拟运行', value: 'simulation' }, { label: '实时数据', value: 'live' }];
const publishedView = ref(false);
...
const sourceManifest = ref<TwinSceneManifest>();
...
const loadModels = async (targetRuntime: TwinRuntime, manifest: TwinSceneManifest) => {
	for (const object of manifest.objects.filter((item) => item.kind === 'model' && item.resourceId)) {
		try {
			...
			await targetRuntime.loadGlbBuffer(object.objectId, resource?.sourceFileName || `${object.name}.glb`, response?.data instanceof ArrayBuffer ? response.data : response);
		} catch (reason: any) { error.value = `模型 ${object.name} 加载失败：${reason?.msg || reason?.message || '未知错误'}`; }
	}
};

const rebuildRuntime = async () => {
	if (!sourceManifest.value || !container.value) return;
	stopPolling();
	runtime?.dispose();
	runtime = undefined;
	selected.value = null;
	...
	const targetRuntime = new TwinRuntime(container.value, runtimeManifest, {
		onSelectionChange: (value) => { selected.value=value; statusVisible.value=Boolean(value?.kind && value.kind!=='route-point'); },
		onMetrics: (value) => Object.assign(metrics, value),
		onError: (message) => { error.value = message; },
	}, { readOnly: true });
	runtime = targetRuntime;
	running.value = runtimeMode.value === 'simulation';
	targetRuntime.setRunning(running.value);
	if(!anchorFrame)anchorFrame=requestAnimationFrame(updateStatusAnchor);
	await loadModels(targetRuntime, runtimeManifest);
	if (runtime !== targetRuntime) { targetRuntime.dispose(); return; }
	if (runtimeMode.value === 'live') startPolling();
};
...
const load = async () => {
	...
	publishedView.value = requestedVersion > 0;
	if (publishedView.value) {
		...
		runtimeMode.value = route.query.runtimeMode === 'live' ? 'live' : 'simulation';
	} else {
		versionNo.value = 0;
		versionLabel.value = `草稿 r${scene.revision} · 只读模拟预览`;
		runtimeMode.value = 'simulation';
	}
	sourceManifest.value = cloneTwinManifest(manifest);
	await nextTick();
	await rebuildRuntime();
};
const poll = async () => {
	if(!sceneId || !publishedView.value || runtimeMode.value !== 'live' || !versionNo.value)return;
	try {
		const snapshot=apiData<TwinRuntimeSnapshot>(await digitalTwinApi.snapshot(sceneId,versionNo.value));
		const updates=snapshot?.updates||[];
		latestUpdates.value=Object.fromEntries(updates.map(update=>[update.bindingId,update]));
		runtime?.applyDataUpdates(updates);
	}catch{/* 下一轮重试 */}
};
...
const changeRuntimeMode = async (value: string | number | boolean) => {
	if (!publishedView.value) return;
	runtimeMode.value = value === 'live' ? 'live' : 'simulation';
	error.value = '';
	await rebuildRuntime();
	void router.replace({ query: { ...route.query, runtimeMode: runtimeMode.value } });
};
const focusSimulationPallets = () => {
	if (runtimeMode.value !== 'simulation') return;
	if (!runtime?.focusSimulationPallets()) error.value = '当前模拟场景没有可聚焦的小托盘';
};
const toggleRunning = () => {
	if (runtimeMode.value !== 'simulation') return;
	running.value = !running.value; runtime?.setRunning(running.value);
};
