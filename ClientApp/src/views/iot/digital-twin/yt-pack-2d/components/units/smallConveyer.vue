<template>
  <g :transform="`translate(${config.x}, ${config.y})`" class="smallConveyer" @click="handleClick()">
    ...
    <!-- 内边 -->
    <g v-if="config.border" fill="#555">
      <rect v-if="config.border.includes('u')" :x="0" :y="0" :width="config.width" :height="5"/>
      <rect v-if="config.border.includes('d')" :x="0" :y="config.height-5" :width="config.width" :height="5"/>
      <rect v-if="config.border.includes('l')" :x="0" :y="0" :width="5" :height="config.height"/>
      <rect v-if="config.border.includes('r')" :x="config.width-5" :y="0" :width="5" :height="config.height"/>
    </g>
...
defineOptions({ name: 'YtSmallConveyer' });
...
const openDialog = inject<(data: UnitInfo) => void>('openSmallConveyerDialog', () => undefined);
