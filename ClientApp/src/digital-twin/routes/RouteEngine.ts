...
export const resolveRoutePath = (source: TwinRouteDefinition, context: TwinRouteRoutingContext = {}): TwinResolvedRoutePath => {
	...
	if (!start) return { points: [], edgeIds: [], closed: false, unavailableEdgeIds: [] };
	...
	for (const edge of route.edges || []) {
		const routeSignalStale = Boolean((edge.blockedBindingId && staleBindingIds.has(edge.blockedBindingId)) || (edge.occupancyBindingId && staleBindingIds.has(edge.occupancyBindingId)));
		const bindingBlocked = edge.blockedBindingId ? isSignalTrue(context.bindingValues?.[edge.blockedBindingId]) : false;
		...
		if (edge.enabled === false || edge.blocked === true || routeSignalStale || bindingBlocked || capacityBlocked || !pointsById.has(edge.fromPointId) || !pointsById.has(edge.toPointId)) {
	...
	for (let step = 0; step < (route.edges?.length || 0) + 1; step += 1) {
		...
		const matchedRule = route.routingMode === 'automatic'
			? (route.decisionRules || [])
				.filter((rule) => rule.enabled !== false && rule.junctionPointId === currentPointId && candidates.some((candidate) => candidate.edge.edgeId === rule.edgeId) && !(rule.source === 'binding' && rule.bindingId && staleBindingIds.has(rule.bindingId)))
	...
	if (!hadConfiguredGraph && pointPath.length < 2 && route.points.length >= 2) return { points: route.points, edgeIds: [], closed: route.loop, unavailableEdgeIds: [...unavailableEdgeIds] };
	return { points: pointPath, edgeIds: edgePath, closed, unavailableEdgeIds: [...unavailableEdgeIds] };
};
...
export class RouteEngine {
	...
	setRunning(running: boolean) {
		this.running = running && this.lengthMeters > 0;
	}
	...
	updateFixed(deltaSeconds: number) {
		this.previousDistance = this.distanceMeters;
		if (!this.running || this.lengthMeters <= 0) return;
		...
		if (this.distanceMeters >= this.lengthMeters) this.running = false;
	}
	...
	getSnapshot(): TwinRouteEngineSnapshot {
		...
		return {
			state: completed ? 'completed' : this.running ? 'running' : 'paused',
	...
	private createCurve(route: TwinRouteDefinition) {
		...
		return new CatmullRomCurve3(vectors, this.pathLoops, 'centripetal', 0.5);
	}
