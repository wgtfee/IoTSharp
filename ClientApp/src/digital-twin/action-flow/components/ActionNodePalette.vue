<template>
	<aside class="af-palette">
		<el-input v-model="search" clearable size="small" aria-label="搜索流程节点" placeholder="搜索流程节点" />
...
import { sceneActionFlowNodeTypes, type TwinActionFlowNodeType } from '../contracts/action-flow-v2';
const props=defineProps<{scene?:boolean}>();
...
const groups:Array<{name:string;items:Array<{type:TwinActionFlowNodeType;label:string}>}>=[
	{name:'三维工艺联动',items:[{type:'WaitStation',label:'等待工位批次到位'},{type:'CompleteStation',label:'完成并放行批次'},{type:'WaitInterlock',label:'等待联锁满足'},{type:'SetState',label:'设置状态/变量'},{type:'MarkMaterial',label:'标记物料工艺阶段'},{type:'Loop',label:'有界批次循环'},{type:'Pick',label:'接近抓取并提升'},{type:'Place',label:'接触放料并回撤'}]},
...
const filteredGroups=computed(()=>{const q=search.value.trim().toLowerCase(); return groups.map(g=>({...g,items:g.items.filter(i=>(!props.scene||sceneActionFlowNodeTypes.has(i.type))&&`${i.label} ${i.type}`.toLowerCase().includes(q))})).filter(g=>g.items.length);});
